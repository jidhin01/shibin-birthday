...
const ShibinTinder = () => {
  ...
  const [matchStatus, setMatchStatus] = useState(null); // null, rejected, match
  const [isModalOpen, setIsModalOpen] = useState(false);

  const profiles = [
    { name: "Beautiful Girl", img: "https://images.unsplash.com/photo-1517841905240-472988babdf9?w=500&q=80" },
    { name: "Senior Developer Role", img: "https://images.unsplash.com/photo-1498050108023-c5249f4df085?w=500&q=80" },
    { name: "Healthy Hairline", img: "https://images.unsplash.com/photo-1560066984-138dadb4c035?w=500&q=80" }
  ];
  ...
  return (
    <div style={{ margin: '60px 0', textAlign: 'center' }}>
      <Button 
        onClick={() => setIsModalOpen(true)}
        className="pulse-btn"
        style={{ background: '#f472b6', color: 'white', padding: '15px 30px', fontSize: '1.2rem' }}
      >
        ❤️ ഷിബിന്റെ ഡേറ്റിംഗ് ആപ്പ് (Dating App)
      </Button>

      <Modal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        title="Shibin-Der (Dating App)"
        size="small"
      >
        <div className="tinder-container">
          <div className="tinder-card">
            <img src={profiles[0].img} alt="Profile" className="tinder-img" />
          ...
          <div className="tinder-actions">
            <button className="tinder-btn nope" onClick={() => handleMatch('nope')}>✖️</button>
            <motion.button 
          ...
          {matchStatus === 'rejected' && (
            <div className="tinder-overlay rejected">
              <h2>അവൾക്ക് നിന്നെ വേണ്ട!</h2>
              <p>നിന്റെ കോഡ് കണ്ടപ്പോൾ തന്നെ അവൾ ഓടിപ്പോയി.</p>
            </div>
          )}

          {matchStatus === 'match' && (
            <div className="tinder-overlay match">
              <h2>ഇതൊരു അത്ഭുതമാണ്!</h2>
              <p>പക്ഷെ ഇത് ശരിക്കുള്ള പെൺകുട്ടിയല്ല, ഇതൊരു 'Error 404' ആണ്.</p>
            </div>
          )}
        </div>
      </Modal>
